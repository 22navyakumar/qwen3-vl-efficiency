...
# Filename patterns
#   <dataset>_<method>_cr<ratio>.jsonl
#   <dataset>_modality-<inner>_i<img_ratio>_t<text_ratio>.jsonl
UNIFORM_RE = re.compile(r"(?P<dataset>[a-z]+)_(?P<method>[a-z]+)_cr(?P<ratio>\dp\d{2})$")
MODALITY_RE = re.compile(
    r"(?P<dataset>[a-z]+)_modality-(?P<inner>[a-z]+)_i(?P<img>\dp\d{2})_t(?P<txt>\dp\d{2})$"
)


def parse_filename(stem: str) -> dict:
    """Pull the experiment metadata back out of the filename."""
...
def aggregate_cell(path: Path) -> dict:
    """Read one JSONL and compute per-cell aggregates."""
    ...
    return {
        ...
        "mean_tput_tok_s": avg("tput_tok_s"),
        "peak_mem_gb": max((s.get("peak_mem_gb", 0) for s in samples), default=None),
...
def main() -> None:
    ...
    cols = [
        ...
        "n_samples", "accuracy",
        "mean_prefill_ms", "mean_decode_ms", "mean_tput_tok_s",
        "peak_mem_gb",
        "mean_image_fraction", "mean_effective_compression",
        "file",
    ]
    df = df[cols].sort_values(by=["dataset", "method", "compression_ratio",
                                  "image_ratio", "text_ratio"]).reset_index(drop=True)
